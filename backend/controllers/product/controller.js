...
const createProduct = TryCatch(async (req, res) => {
  const { name, categoryId, price, description, ref, imageUrl, model, stock } =
    req.body;
...
const deleteProduct = TryCatch(async (req, res) => {
  ...
  const isInInvoice = await invoiceModel
    .find({ $and: { "products.product": productId } })
    .countDocuments();
  const isInLead = await leadModel
    .find({ $and: { products: productId } })
    .countDocuments();
  const isInOffers = await offerModel
    .find({ $and: { "products.product": productId } })
    .countDocuments();
  const isInProformaInvoices = await proformaInvoiceModel
    .find({ $and: { "products.product": productId } })
    .countDocuments();
  ...
  return res.status(400).json({
    ...
    success: false,
    message:
      "Product cannot be deleted as it is being used in Invoices, Proforma Invoices, Leads or Offers",
  });
});
