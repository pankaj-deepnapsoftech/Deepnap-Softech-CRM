...
const editAdminAccess = TryCatch(async (req, res) => {
  ...
  modifiedPermissions = permissions.filter(
    (permission) =>
      permission !== "admin" &&
      permission !== "website-configuration" &&
      permission !== "settings"
  );
...
const changeSuperAdminDetails = TryCatch(async (req, res) => {
  const { _id, name, phone, email } = req.body;

  const isExistingUser = await adminModel.findById(_id);
  if (!isExistingUser) {
    throw new Error("User not found", 404);
  }
  if (isExistingUser.role !== "Super Admin") {
    throw new Error(
      "Editing account details is only available for Super Admin",
      401
    );
  }

  await adminModel.findOneAndUpdate({ _id }, { name, email, phone });

  res.status(200).json({
    status: 200,
    success: true,
    message: "Account details has been changed successfully",
  });
});
